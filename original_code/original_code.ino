...
//long server_ok,server_ng,server_by_pass;
long sd_ok,sd_ng,sd_by_pass;
//long updated_ok,updated_ng,updated_by_pass;
/* -----------------------------IO Variables--------------------------------------*/
int ip1,ip2,ip3,ip4;
int op1,op2;
/* -----------------------------WiFi Credentials---------------------------------------*/
...
const char* serverUrl = "http://192.168.2.101/api/api/dataview.php?device_id=Device_123456";
...
//Function to get Data from server and store the value in seprate variable
// void getData() {
//   long server_ok,server_ng,server_by_pass;
//   HTTPClient http;
...
//        server_ok = doc[0]["ok"];
//        server_ng = doc[0]["ng"];
//        server_by_pass = doc[0]["by_pass"];
//      Serial.print("Server ok: ");
//      Serial.println(server_ok);
//      Serial.print("Server ng: ");
//      Serial.println(server_ng);
//      Serial.print("Server by_pass: ");
//      Serial.println(server_by_pass);
//     }
//   } else {
...
void update_data_from_sdcard(fs::FS &fs, const char * path)
{
   //long sd_ok,sd_ng,sd_by_pass;
   char *card;
   String first_line;
  ...
  while (file.available()) {
    ...
    //Serial.println("ashish");
    //Serial.println(first_line);   
     
    break;
  }
 file.close();
 card = strtok(&first_line[0],",");
 stc++;
 //long sd_ok,sd_ng,sd_by_pass;
 while (card != NULL)
  {
...
void edge_data_sync_req(long ok_1,long ng_1,long by_pass_1)
{
  ...
  if( data_update_flag==1){
    ...
    if(WiFi.status()== WL_CONNECTED){
      String serverPath = "http://192.168.2.101/api/api/data.php?ok=" + (String)ok1 +"&ng=" + (String)ng1+ "&by_pass=" + (String)by_pass1+ "&mac_id=" + (String)MAC_ID+"&actual_cycle_time="+(String)diff_time+"&device_date="+(String)c_date+"&device_time="+(String)c_time;
    ...
    /* if(WiFi.status()== WL_CONNECTED){
      String serverPath2 = "http://192.168.2.101/api/api/time_data.php?cycle_time=" + (String)diff_time +"&idle_time=" + (String)idle_time+ "&mac_id=" + (String)MAC_ID;
...
void device_started(int code1,int code2)
{
  ...
  String file_path;
 if(WiFi.status()== WL_CONNECTED){
      String serverPath = "http://192.168.2.101/api/api/device_boot.php?mac_id=" +(String)MAC_ID+"&start_type=SYSTEM_START_C1:"+(String)code1+"_C2:"+(String)code2+"&device_date="+(String)c_date+"&device_time="+(String)c_time;
      //Serial.println(serverPath);
      jsonBuffer = httpGETRequest(serverPath.c_str());
      //Serial.println(jsonBuffer);
    }
...
void delete_SD_data_files()
 ...
 void machine_operation()
 {
  ...
  if(WiFi.status()== WL_CONNECTED){
      String serverPath = "http://192.168.2.101/api/api/machine.php?mac_id=" +(String)MAC_ID+"&disk="+memory+"&temp="+(String)temp+"&memory=340kb"+"&device_date="+(String)c_date+"&device_time="+(String)c_time+"&rssi="+(String)rssi+"&op=device_health_status"+"&status=1";
      //Serial.println(serverPath);
      jsonBuffer = httpGETRequest(serverPath.c_str());
      //Serial.println(jsonBuffer);
    }
...
if(strcmp(&jsonBuffer[0],"1111")==0)
{
  ...
  String serverPath = "http://192.168.2.101/api/api/machine.php?mac_id=" + (String)MAC_ID+"&op=machine_ON_command&status=1";
  //Serial.println(serverPath);
  jsonBuffer = httpGETRequest(serverPath.c_str());
}
else if(strcmp(&jsonBuffer[0],"0000")==0)
{
  ...
  String serverPath = "http://192.168.2.101/api/api/machine.php?mac_id=" + (String)MAC_ID+"&op=machine_OFF_command&status=1";
  //Serial.println(serverPath);
  jsonBuffer = httpGETRequest(serverPath.c_str());
}
else if(strcmp(&jsonBuffer[0],"2222")==0)
{
  if(WiFi.status()== WL_CONNECTED){
      ...
      String serverPath2 = "http://192.168.2.101/api/api/ftp_data.php?mac_id="+(String)MAC_ID+"&op=ftp_file_upload&status=init";
      //Serial.println(serverPath2);
      ...
      if(complete==1)
      {
      serverPath2 = "http://192.168.2.101/api/api/ftp_data.php?mac_id="+(String)MAC_ID+"&op=ftp_file_upload&status=1";
      //Serial.println(serverPath2);
      ftp_data_date = httpGETRequest(serverPath2.c_str());
      }
      else if(complete==-1)
      ...
      {
      serverPath2 = "http://192.168.2.101/api/api/ftp_data.php?mac_id=" +(String)MAC_ID+"&op=ftp_file_upload&status=0";
      Serial.println(serverPath2);
      ftp_data_date = httpGETRequest(serverPath2.c_str());
      }
      }
...
{
      String ftp_data_date;
      String serverPath2 = "http://192.168.2.101/api/api/ftp_data.php?mac_id=" +(String)MAC_ID+"&op=file_delete&status=init";
      //Serial.println(serverPath2);
      ftp_data_date = httpGETRequest(serverPath2.c_str());
      //Serial.println(ftp_data_date);
      if(strcmp(&ftp_data_date[0],"0000")!=0){
      ...
      if(complete==3)
        {
        serverPath2 = "http://192.168.2.101/api/api/ftp_data.php?mac_id=" +(String)MAC_ID+"&op=file_delete&status=3";
        Serial.println(serverPath2);
        ftp_data_date = httpGETRequest(serverPath2.c_str());
        complete=0;
        }
        ...
        {
        serverPath2 = "http://192.168.2.101/api/api/ftp_data.php?mac_id=" +(String)MAC_ID+"&op=file_delete&status=2";
        Serial.println(serverPath2);
        ftp_data_date = httpGETRequest(serverPath2.c_str());
        complete=0;
        }
        ...
        {
        serverPath2 = "http://192.168.2.101/api/api/ftp_data.php?mac_id=" +(String)MAC_ID+"&op=file_delete&status=1";
        //Serial.println(serverPath2);
        ftp_data_date = httpGETRequest(serverPath2.c_str());
        complete=0;
        }
...
//String folderName;
folderName = "/IIOT_"+c_month;
file_path3=folderName+"/production_data_"+c_date+".csv";
update_data_from_sdcard(SD,&file_path3[0]);
 Serial.print("sd_OK=");
 Serial.println(sd_ok);
 Serial.print("sd_NG=");
 Serial.println(sd_ng);
 Serial.print("sd_by_pass=");
 Serial.println(sd_by_pass);
delay(100);
