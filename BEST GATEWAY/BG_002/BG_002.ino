...
long counter,ok,ng,by_pass,data_serial;
long prev_ok,prev_ng,prev_by_pass,prev_data_serial;
int rtc_update_flag;
String status_flag = "";
...
IPAddress local_IP(192, 168, 2, 16);
...
void update_data_from_mmc(fs::FS &fs, const char * path)
{
   ...
   char data_date[11];
   char data_time[9];
   int cycle_time, idle_time,data_sync_status;
   char data_datetime[20];
   File file = fs.open(path);
   if(!file){
        Serial.println("Failed to open file for reading");
        return;
  ...
  file.close();
  char* token = strtok(&first_line[0], ",");
  while (token != NULL) {
        switch (index) {
            case 0: data_serial = atoi(token);
                    break;
            case 1: strcpy(data_date, token);
                    break;
            case 2: strcpy(data_time, token);
                    break;
            case 3: ok = atoi(token);
                    break;
            case 4: ng = atoi(token);
                    break;
            case 5: by_pass = atoi(token);
                    break;
            case 6: cycle_time = atoi(token);
                    break;
            case 7: idle_time = atoi(token);
                    break;
            case 8: strcpy(data_datetime, token);
                    break;
            case 9: data_sync_status = atoi(token);
                    break;
        }
...
void append_data_historian(long data_serial1,long ok1,long ng1, long by_pass1,String date1,String time1, String time_stamp1,int cycle_time1, int idle_time1,int sync1)
{
String data_string;
String file_path;
file_path="/historian_data_"+c_date+".csv";
//Serial.println(file_path);
data_string=(String)data_serial1+","+date1+","+time1+","+(String)ok1+","+(String)ng1+","+(String)by_pass1+","+(String)cycle_time1+","+(String)idle_time1+","+time_stamp1+","+(String)sync1+"\n";
//Serial.println(data_string);
appendFile(SD,&file_path[0],&data_string[0]);  
}

//Add data to production log datewise
void add_data_to_production_log(long data_serial1,long ok1,long ng1, long by_pass1,String date1,String time1, String time_stamp1,int cycle_time1, int idle_time1,int sync1)
{
String data_string;
String file_path;
...
void edge_data_sync_req(long data_serial_1,long ok_1,long ng_1,long by_pass_1)
{
   time(&end_time);
   int sync1;
   long ok1,ng1,by_pass1,data_serial1;
   ok1=ok_1;
   ng1=ng_1; 
   by_pass1=by_pass_1;
   data_serial1=data_serial_1;
  ...
  if(strcmp(&prev_date[0],&c_date[0])!=0)
  {
   ...
   data_serial1=data_serial1-prev_data_serial;
   prev_date=c_date;
   ok=ok1;
   ng=ng1;
   by_pass=by_pass1;
  ...
  else
  {
     ...
     if(prev_ok>ok1 || prev_ng>ng1 || prev_by_pass>by_pass1)
     {
        String file_path2;
        ...
        if(SD.exists(&file_path2[0]))
        {
          ...
          data_serial1=data_serial + data_serial1;
          ok=ok1;
          ng=ng1;
          by_pass=by_pass1;
          data_serial=data_serial1;
     ...
     else
      {
          ...
          data_serial1=prev_data_serial + data_serial1;
          ok=ok1;
          ng=ng1;
          by_pass=by_pass1;
          data_serial=data_serial1;
  ...
  if( data_update_flag==1){
    ...
    if(WiFi.status()== WL_CONNECTED){
      String serverPath = "http://192.168.3.19/eol/module/autoprn/API/api/data.php?data_serial=" + (String)data_serial1+"&ok=" + (String)ok1 +"&ng=" + (String)ng1+ "&by_pass=" + (String)by_pass1+ "&mac_id=" + (String)MAC_ID+"&actual_cycle_time="+(String)diff_time+"&device_date="+(String)c_date+"&device_time="+(String)c_time+"&status="+(String)status_flag;
      //String serverPath = "http://192.168.2.2/autoprn/API/api/data.php?data_serial=" + (String)data_serial1+"ok=" + (String)ok1 +"&ng=" + (String)ng1+ "&by_pass=" + (String)by_pass1+ "&mac_id=" + (String)MAC_ID+"&actual_cycle_time="+(String)diff_time+"&device_date="+(String)c_date+"&device_time="+(String)c_time+"&status="+(String)status_flag;
  ...
  append_data_historian(data_serial1,ok1,ng1,by_pass1,c_date,c_time,c_timestamp,diff_time,100,sync1);
  add_data_to_production_log(data_serial1,ok1,ng1,by_pass1,c_date,c_time,c_timestamp,diff_time,100,sync1);
   }
...
//OTA SERVER Init
 server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
  ...
  AsyncElegantOTA.begin(&server, "BG_002", "200_GB");    // Start ElegantOTA
  ...
  for(;;){
    ...
    if(data_update_flag==1)
    {
      Serial.println("New Data!"); 
       edge_data_sync_req(data_serial,ok,ng,by_pass);
       time(&start_time);
       data_update_flag=0;
    }
    else
...
/* -----------------------------Signal Data Logic Starts From Here---------------------------------------*/
  ...
  if(prod_start_flag==1)
  {
    ...
    else if(ip4==1 && ip2==1)
    {
      ...
      Serial.print("Part is Processing");
    }            
        }
